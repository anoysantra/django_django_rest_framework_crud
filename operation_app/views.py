    ...
    if request.method == 'POST':
        ...
        if cricketer_serializer.is_valid():
            ...
            if batting_record_serializer.is_valid():
                ...
                return errors_response('batting', batting_record_serializer)
            ...
            if bowling_record_serializer.is_valid():
                ...
                return errors_response('bowling', bowling_record_serializer)

            return Response({'message': 'Cricketer data saved successfully'})

        else:
            return errors_response('cricketer', cricketer_serializer)

    else:
        return Response({'message': 'Method not allowed'}, status=status.HTTP_405_METHOD_NOT_ALLOWED)


def errors_response(name, specific_serializer):
    errors = {}
    errors[name] = [
        {'field': field, 'message': msg}
        for field, msg in specific_serializer.errors.items()
    ]
...
def update_delete_cricketer(request, player_id):
    ...
    if request.method == 'POST':
        cricketer_serializer = CricketerSerializer(
            instance=cricketer, data=request.data)

        old_player_id = cricketer.player_id

        if cricketer_serializer.is_valid():
            new_player_id = cricketer_serializer.validated_data.get(
                'player_id')
...
"""
def debug(request):
    player = Cricketers.objects.get(player_id='P001')
    # for player in players:
    batting_stats = player.get_batting_stats()
    bowling_stats = player.get_bowling_stats()
    # Debug print
    print(
        f"Batting: {batting_stats.batting_style}, Bowling: {bowling_stats.bowling_style}")
